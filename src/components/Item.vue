<template>
  <div>
    ...
    <p>Recipes: <span v-for="recipe in item.recipe" :key="recipe">{{recipe}} </span></p>
    <p>Recipe Price: {{item.recipePrice}}</p>

    <h2>Stat</h2>
    <ul>
     <li v-for="stat in item.stats" :key="stat.name">{{stat.name}}: <i>{{stat.value}}</i></li>
    </ul>
    ...
    <div v-for="ability in item.abilities" :key="ability.name">
      ...
      <ul>
        <li v-for="attr in ability.attributes" :key="attr.name">{{attr.name}}: <i>{{attr.value}}</i></li>
      </ul>
    </div>
